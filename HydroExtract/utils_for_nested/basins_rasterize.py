...
# basin的geojson文件栅格化: shp文件路径 参考Raster文件路径 结果输出路径 结果范围参考(默认参考tif)
def basin_rasterize(geoj_path, raster_path, result_path, value, no_data, shp_ex=0):
    ...
    re_ds = driver.Create(result_path, raster_x_size, raster_y_size, 1, gdal.GDT_Float32, options=['COMPRESS=DEFLATE'])
